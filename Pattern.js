 ...
 */ export default function Pattern(props) {
  ...
  const xScale =
    direction === "left" || direction === "right" ? factor * scale : 0;
  const yScale =
    direction === "top" || direction === "bottom" ? factor * scale : 0;
  if (patternType === "wave") {
    pattern = `radial-gradient(circle at center center, ${front}, ${back}), repeating-radial-gradient(circle at center center, ${front}, ${front}, ${scale}px, transparent ${
      scale * 2
  ...
  if (patternType === "diagonal") {
    pattern = `repeating-linear-gradient(45deg, ${front}, ${front} ${
      scale / 2
    }px, ${back} ${scale / 2}px, ${back} ${scale * 2}px )`;
  }
  if (patternType === "diagonal-two") {
    pattern = `repeating-linear-gradient( 135deg, ${front}, ${front} ${
      scale / 2
    }px, ${back} ${scale / 2}px, ${back} ${scale * 2}px )`;
  }
  if (patternType === "blocks") {
    ...
    position = `0 0, 0 0, ${scale * 4}px ${scale * 7}px, ${scale * 4}px ${
      scale * 7
    }px, 0 0, ${scale * 4}px ${scale * 7}px`;
  }
  ...
  if (patternType === "paper") {
    ...
    size = `${scale * 5}px ${scale * 5}px, ${scale * 5}px ${
      scale * 5
    }px, ${scale}px ${scale}px, ${scale}px ${scale}px`;
...
/* Property Controls */ addPropertyControls(Pattern, {
  patternType: {
  ...
  shouldAnimate: {
    ...
    hidden(props) {
      return (
        props.patternType !== "wave-lines" &&
        props.patternType !== "wiggle" &&
        props.patternType !== "clouds" &&
        props.patternType !== "stars" &&
        props.patternType !== "groovy"
      );
    },
  },
  direction: {
